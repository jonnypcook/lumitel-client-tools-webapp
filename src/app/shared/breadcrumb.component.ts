...
@Component({
    ...
    template: `
  <ng-template *ngIf="available()" ngFor let-breadcrumb [ngForOf]="breadcrumbs" let-last = last>
...
export class BreadcrumbsComponent {
    ...
    constructor(private router: Router, private route: ActivatedRoute, private capitalizeWordsPipe: CapitalizeWordsPipe) {
    }
                ...
                childrenRoutes.forEach(route => {
                    if (route.outlet === 'primary') {
                        ...
                        if (!!routeSnapshot.params['iid'] && rUrl.match(/^[\d]+$/) && lastRouteName === 'installation') {
                            ...
                            if (routeSnapshot.params['type'] && route.snapshot.data.title.match(/^devices$/i)) {
                                route.snapshot.data.title = this.capitalizeWordsPipe.transform(routeSnapshot.params['type']) + ' ' + route.snapshot.data.title;
                            }
                            this.breadcrumbs.push({
